///
/// AccessibilityDialog v1.0.4 (for VW, using Tailwind Classes)
///
class AccessibilityDialog extends HTMLElement {
  ...
  _defaultConfig = {
    ...
    Settings: [
      ...
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-easy-layout" }],
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-hide-image" }],
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-high-contrast" }],
      },
      {
        Key: "SettingHeadlineBigger",
        Label: "",
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-headline-l" }],
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-highlight-links" }],
      },
      {
        Key: "SettingHighlightButtons",
        Label: "",
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-highlight-buttons" }],
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-highlight-form" }],
      },
      {
        Key: "SettingDgs",
        Label: "",
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-dgs-video" }],
      },
      {
        Key: "SettingAudio",
        Label: "",
        Enabled: false,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-audio" }],
      },
      {
        Key: "SettingAudioRate",
        Label: "Vorlesen Abspielgeschwindigkeit",
        Type: "radio",
        Enabled: false,
        Values: [
          { Label: "70%", Value: "access-audio-rate-70", Group: "audio-rate" },
          { Label: "80%", Value: "access-audio-rate-80", Group: "audio-rate" },
          { Label: "90%", Value: "access-audio-rate-90", Group: "audio-rate" },
          {
            Label: "100%",
            Value: "access-audio-rate-100",
            Group: "audio-rate",
          },
          {
            Label: "110%",
            Value: "access-audio-rate-110",
            Group: "audio-rate",
          },
          {
            Label: "120%",
            Value: "access-audio-rate-120",
            Group: "audio-rate",
          },
        ],
      },
      {
        ...
        Enabled: true,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-dialog-feedback" }],
      },
      {
      ...
      },
      {
        ...
        Enabled: false,
        Type: "checkbox",
        Values: [{ Label: "", Value: "access-open-dyslexic-font" }],
      },
      {
		...
		<div class="accessibility-dialog">
			<div class="accessbility-setting-container">
				<div class="a11y-icon-container">
					...
					<button class="acc-dialog-close close-btn js-close-modal" aria-label='${
            this._labels.LabelClose
				...
				<h2 class="accessibility-dialog-header">${
          this._labels.AccessibilityDialogHeader
        }</h2>
				<div class="menue">
					<button class="js-reset reset-btn flex flex-row items-center justify-center" aria-label='${
            this._labels.ResetAllProfiles
          }'>
					<svg id="zurücksetzen" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 21.051 24.169" fill="white" class="me-3">
          ...
          this._config.Settings != undefined && this._config.Settings.length > 0
			...
			<div class="accessbility-close-container">
				<button class="reset-btn reset-btn-sticky js-close-modal d-flex align-items-center justify-content-center"
					...
					<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24.582 24.582" fill="black" class="me-3">
						<g id="schließen" transform="translate(1.061 1.061)">
							<line data-name="Linie 5" x2="22.46" y2="22.46" fill="none" stroke="#fff" stroke-width="3"/>
							<line data-name="Linie 6" x1="22.46" y2="22.46" fill="none" stroke="#fff" stroke-width="3"/>
						</g>
					</svg>
					${this._labels.LabelClose}
				</button>
    ...
    return `<div class="setting-wrapper">
            ...
            <div class="me-3">
              <input id="${
                ...
                aria-label='${
                  this._labels.DialogAriaLabelProfile
                } ${this._getLabel(profile.Name)} ${this._labels.DialogSelect}'
              >
              <span class="toggle-switch js-toggle-setting" aria-labelledby="${
                profile.Id
              }" tabindex="0"></span>
            </div>
            <div clas="d-flex flex-column">
				      <span class="toggle-label ml-0 font-bold"
  ...
  _renderRadioSetting(settingDef) {
    let i = 0;
    return `<div class="setting-wrapper" data-setting-number="${
      this.settingNumber
    }">
		<div class="subheader"
      ...
      <fieldset>
        ...
        ${settingDef.Values.map((setting) => {
          let result = `<div class="mb-2 bg-gray-200 hover:bg-gray-300 rounded hover:cursor-pointer">
							<label class="w-full flex items-center hover:cursor-pointer px-3 py-2" for="${
                setting.Group
              }-${i}">
							<input type="radio" id="${
                setting.Group
              }-${i}" class="js-setting hover:cursor-pointer me-2" name="${
            setting.Group
          }"
  ...
  _renderCheckboxSetting(settingDef) {
    return `<div class="setting-wrapper" data-setting-number="${
      ...
      <div class="toggle-entry">
        <label class="toggle" aria-label="${
          this._labels.DialogAriaLabelSetting
        } ${this._getLabel(setting.Label)} ${this._labels.DialogSelect}">
  ...
  _onResetSetting(e) {
    ...
    if (settingWrapper) {
      ...
      if (selectedSetting) {
        ...
        let bodyClasses = Array.from(document.documentElement.classList);
  ...
  _onSettingChange(e) {
    ...
    const elemSetting = element.getAttribute(this._dataSettingsAttribute);
    ...
    if (elemSetting) {
      ...
      if (element.type === "checkbox") {
        ...
        if (element.checked && !this._currentSettings.includes(elemSetting)) {
          this._currentSettings.push(elemSetting);
        }
      }
  ...
  _removeCbFromAppliedSettingsAndReset(target) {
    ...
    ).filter((c) => c !== target.getAttribute(this._dataSettingsAttribute));
    this._setBodyClasses(newBodyClasses);
  }
  ...
  _setBodyClasses(bodyClasses) {
    ...
    document.documentElement.classList = bodyClassesString;
  }
          ...
          if (el != null && el.checked) {
            ...
            if (el.type === "radio") {
              ...
              if (activeSetting.length === 0) {
                ...
                const targetElement = this._allSettings.querySelector(
  ...
  _storeAccessibilityColorSettings(colorSetting) {
    ...
    if (
      this._savePath &&
      this._isSettingsChangeDone &&
      this._isProfileChangeDone
    ) {
      const data = {
  ...
  _onHideImages(e) {
    ...
    if (images && images.length > 0) {
      ...
      for (let i = 0; i < images_length; i++) {
        if (!headerSection.contains(images[i])) {
  ...
  _beepOpen() {
    if (document.documentElement.classList.contains("access-dialog-feedback")) {
      var beep = new Audio(
  ...
  _setStoredSettings(accessibilitySettings) {
    ...
    if (
      this._savePath &&
      this._isProfileChangeDone &&
      this._isColorSettingsChangeDone
    ) {
      const data = {
        [this._settingsKey]: JSON.stringify(this._currentSettings),
        [this._profileKey]: JSON.stringify(this._currentProfiles),
